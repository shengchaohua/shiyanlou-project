# -*- coding: utf-8 -*-
"""
Created on Sat Apr 14 09:46:39 2018

@problem: leetcode 811. Subdomain Visit Count
...
class Solution:
    def subdomainVisits(self, cpdomains):
        ...
        for cpd in cpdomains:
            freq_domains = cpd.split(' ')
            freqs.append(int(freq_domains[0]))
            domains.append(freq_domains[1])
        length = len(domains)
        for i in range(length):
            d = domains[i]
            d_s = d.split('.')
            d_s_2 = d_s[-1]
            self._add(freqs, domains, d_s_2, i)
            
            if len(d_s) == 3:
                d_s_3 = d_s[1] + '.' + d_s[2]
                self._add(freqs, domains, d_s_3, i)
                
                    
        return [str(f) + ' ' + d for f, d in zip(freqs, domains)]
    
    def _add(self, freqs, domains, d, i):
        if d not in domains:
            domains.append(d)
            freqs.append(freqs[i])
        else:
            freqs[domains.index(d)] += freqs[i]
            
    
    def subdomainsVisitsAC(self, cpdomains):
        # leetcode AC!
        import collections
